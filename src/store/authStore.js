...
export const useAuth = create((set) => ({
  ...
  login: async (userCred) => {
    ...
    try {
      ...
      //make api call
      let res = await axios.post("https://blog-app-backend-erzv.onrender.com/auth/login", userCred, { withCredentials: true });
      //update state
      if (res.status === 200) {
        set({
  ...
  checkAuth: async () => {
    try {
      set({ loading: true });
      const res = await axios.get("https://blog-app-backend-erzv.onrender.com/auth/check-auth", { withCredentials: true });
